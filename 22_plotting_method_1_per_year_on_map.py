...
# Get longitude and latitude values from the groundwater dataset
lon = groundwater_ds.variables['lon'][:]
lat = groundwater_ds.variables['lat'][:]

# Loop over the years and create a separate plot for each year
for year in range(1980, 2016):  # 1980 to 2015 inclusive
    if year in years_groundwater and year in years_withdrawal:
        ...
        # Ensure the shape matches and calculate energy
        if yearly_mean_per_pixel.shape == Q.shape:
            yearly_energy_per_pixel = calculate_energy(yearly_mean_per_pixel, Q, pump_efficiency)
            
            # Mask out zero values to exclude from plotting
            yearly_energy_per_pixel[yearly_energy_per_pixel == 0] = np.nan
            ...
            mesh = ax.pcolormesh(lon, lat, yearly_energy_per_pixel, cmap='viridis', transform=ccrs.PlateCarree(), vmin=0.001, vmax=1)

            # Add colorbar
            cbar = plt.colorbar(mesh, ax=ax, orientation='horizontal', pad=0.05)
            cbar.set_label(f'Energy Consumption [MWh] for {year}', fontsize=16)
            cbar.ax.tick_params(labelsize=12)

            # Add gridlines and labels
            ax.set_title(f'Groundwater Pump Energy Consumption for Irrigation in {year}', fontsize=18, pad=20)
